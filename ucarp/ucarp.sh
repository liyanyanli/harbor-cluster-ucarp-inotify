...
#./ucarp.sh /root/ha/docker-harbor-cluster/release_harbor/build/harbor 40 love 10.10.103.222 10.10.103.141 eth0 10.10.103.150
# $1 path $2 ucarp id (1~255) $3 ucarp password $4 VIP $5 ip  $6 eth0 $7 other ip

cd $1/build/ucarp
#apt-get -y install ucarp #need to change
#sudo dpkg --install ../../release/ucarp/*.deb

cp ucarp /usr/sbin/ucarp

cp libpcap.so.0.8 /usr/lib64/libpcap.so.0.8
...
mater_up="#!/bin/bash\n/bin/sh /etc/uuarp/inotify.sh $7 /data &\niptables -F\n/bin/sh $harbor_upP\n/sbin/ip addr add $4/24 dev $6\nsleep 10\ncd /etc/uuarp \nnohup ./healthCheck >/dev/null 2>&1 </dev/null &"
...
if [ $? -ne 0 ]
then
    echo "sh $masterP" >> /etc/rc.d/rc.local
